...
limpia_tweets <- function(tweet) {
  ...
  tweet %>%
    ...
    str_replace_all('[ \t]{2,}', '') %>%  str_replace_all('^\\s+|\\s+$', '') %>%
...
```{r}
emotions <- get_nrc_sentiment(tweet_df$text)
